...
import '../datasource/Model/save_subscription_type_body.dart';
...
class KistyRepoImpl implements KistyRepository {
  ...
  @override
  Future<ApiResult<String?>> saveKistyType({
    required int id,
    required String typeName,
    required int amount,
    required int crTypeId,
    ...
    try {
      ...
      final Map userJson = jsonDecode(userData!);
      final result = await kistyRemoteSrc.saveKistyType(
        token: 'Bearer $token',
        kistiTypeBody: KistiSaveModel(
          id: id,
          typeName: typeName,
          amount: amount,
          compId: userJson['cid'],
  ...
  @override
  Future<ApiResult<String?>> addSubsctiptionType({
    required int id,
    required int crTypeId,
    required String typeName,
    required int projectId,
    ...
    try {
      ...
      final Map userJson = jsonDecode(userData!);
      final result = await kistyRemoteSrc.saveSubscriptionTypeData(
        token: 'Bearer $token',
  ...
  @override
  Future<ApiResult<List<GetSubscriptionTypeModel>?>> getSubsctipTypeData() async {
    try {
      final token = await localDataSource.getToken();
      final userData = await localDataSource.getUserInformation();
      if ((token?.isEmpty ?? true) && (userData?.isEmpty ?? true)) {
      ...
      final Map userJson = jsonDecode(userData!);
      final result = await kistyRemoteSrc.getSubscriptionTypeData('Bearer $token', userJson['cid']);
      return result.when(
        success: (data) {
          if (data != null) {
            return ApiResult.success(data);
